...
var VALID_OSC_TYPES: Record<string, boolean> = { sine: true, square: true, sawtooth: true, triangle: true };

function safeOscType(t: string | undefined, fallback: string): OscillatorType {
  if (t && VALID_OSC_TYPES[t]) return t as OscillatorType;
  return fallback as OscillatorType;
}

function safeFreq(f: number, fallback?: number): number {
  if (!f || !isFinite(f) || f <= 0) return fallback || 440;
  return Math.min(f, 22000);
}

function safeDur(d: number | undefined, fallback: number, max: number): number {
  var v = d || fallback;
  if (!isFinite(v) || v <= 0) return fallback;
  return Math.min(v, max);
}

function safeVol(v: number | undefined, fallback: number): number {
  if (v === undefined || v === null || !isFinite(v) || v <= 0) return fallback;
  return Math.min(v, 1);
}
...
export function noise(d?: number, v?: number): void {
  try {
    ...
    var a = ensureAudio();
    if (!a) return;
    var sz = Math.max(1, Math.floor(a.sampleRate * d)),
...
export function breath(d?: number, fq?: number, q?: number): void {
  try {
    d = safeDur(d, 2, 10);
    fq = safeFreq(fq || 800, 800);
    q = q || 5;
    if (!isFinite(q) || q < 0.1) q = 5;
    var a = ensureAudio();
    if (!a) return;
    var sz = Math.max(1, Math.floor(a.sampleRate * d)),
      buf = a.createBuffer(1, sz, a.sampleRate),
    ...
    for (var i = 0; i < count; i++) {
      (function (j: number) {
        ids.push(setTimeout(function () {
          ...
          sound(f, dur, types[Math.floor(Math.random() * types.length)]);
        }, j * 125));
      })(i);
    }
    var stop = function () { ids.forEach(function (id: any) { clearTimeout(id); }); };
